...
function HomeScreen({
  ...
  const date =
    route.params && route.params.date
      ...
      : new Date(); // route.params.date가 있으면 해당 날짜로, 없으면 오늘 날짜로 설정
  const today = getToday(date);
  const tomorrow = getTommorrow(getToday(date));
  const todosToday = todos.filter(
    todo =>
      todo.createdAt?.toDate() >= today && todo.createdAt?.toDate() < tomorrow,
  );
  ...
  console.log(
    '날짜비교: ',
    date.getTime(),
    today.getTime() != getToday(new Date()).getTime(),
  );
  ...
  const onInsertTodo = async trimedText => {
    ...
    if (trimedText && trimedText.length > 3) {
      // 최소 글자수 제한
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <Text style={styles.guideText}>
              할일 "{todoToRemove.title}"을 제거하시겠습니까?
            </Text>
